...
export default class PaymentsAppsClient {
  ...
  async resolveSession({ id, gid }) {
    ...
    return response[this.resolveMutation];
  }
   ...
   * @returns the response body from the Shopify Payments Apps API
   */
  async rejectSession({ id, gid }, reason='') {
    const response = await this.#perform(schema[this.rejectMutation], {
      id: gid,
      reason: {
        code: "PROCESSING_ERROR",
        merchantMessage: `The session was rejected. ${reason}`
      }
    })
    return response[this.rejectMutation];
  }
  ...
  async pendSession({ id, gid }) {
    ...
    return response[this.pendingMutation];
  }
  ...
  async #perform(query, variables) {
    ...
    return response.ok ? JSON.parse(responseBody).data : null
  }
